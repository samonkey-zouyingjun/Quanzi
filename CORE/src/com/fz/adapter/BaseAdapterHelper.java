...
import com.fz.utils.BitmapUtil;
...
public class BaseAdapterHelper
{
	...
	{
		if (convertView == null) { return new BaseAdapterHelper(context, parent, layoutId, position); }
	...
	/**
	 * This method allows you to retrieve a view and perform custom operations on it, not covered by the
	 * BaseAdapterHelper.<br/>
...
//		Picasso.with(context).load(imageUrl).into(view);
		...
		bitmap.setExitTasksEarly(false);
		bitmap.display(view, imageUrl);
		return this;
	}
	
	
	public BaseAdapterHelper setImageUrl(int viewId, String imageUrl,int defaultid,boolean isCicle)
	{
		ImageView view = retrieveView(viewId);
		if (TextUtils.isEmpty(imageUrl))
		{
			if (isCicle)
			{
				...
				view.setImageBitmap(BitmapUtil.toRoundCorner(bitmap));
				bitmap.recycle();
			}
			else {
		...
		FinalBitmap bitmap = FinalBitmap.create(context);
		bitmap.setExitTasksEarly(false);
		bitmap.display(view, imageUrl,isCicle);
		return this;
	}
	 ...
	 * @return The BaseAdapterHelper for chaining.
	 */
	public BaseAdapterHelper setImageUrl(int viewId, String imageUrl,int defaultid)
	{
		ImageView view = retrieveView(viewId);
		if (TextUtils.isEmpty(imageUrl))
		{
		...
		FinalBitmap bitmap = FinalBitmap.create(context);
		bitmap.setExitTasksEarly(false);
		bitmap.configLoadfailImage(defaultid);
		bitmap.configLoadingImage(defaultid);
		bitmap.display(view, imageUrl);
		return this;
	}
	...
	{
		if (position == -1) throw new IllegalStateException("Use BaseAdapterHelper constructor "
