...
import com.fz.afinal.http.FzHttpHandler;
import com.fz.afinal.http.RetryHandler;
import com.fz.afinal.http.SyncRequestHandler;
import com.fz.listener.FzCallBack;
import com.fz.utils.FzConfig;

/**
 * @author cate 2015-2-4 下午2:24:52
 */
public class FzHttpReq {

	private static final int DEFAULT_SOCKET_BUFFER_SIZE = 8 * 1024; // 8KB
	private static final String HEADER_ACCEPT_ENCODING = "Accept-Encoding";
	private static final String ENCODING_GZIP = "gzip";

	private static int maxConnections = FzConfig.Request.maxConnections; // http请求最大并发连接数
	private static int socketTimeout = FzConfig.Request.socketTimeout; // 超时时间，默认10秒
	private static int maxRetries = FzConfig.Request.maxRetries;// 错误尝试次数，错误异常表请在RetryHandler添加
	private static int httpThreadCount = FzConfig.Request.httpThreadCount;// http线程池数量
	private final DefaultHttpClient httpClient;
	private final HttpContext httpContext;
	private String charset = FzConfig.Request.charset;

	private final Map<String, String> clientHeaderMap;
	
	
//	private FzHttpHandler mHandler;
	...
	// ------------------get 请求-----------------------
	public <T> void get(String url, FzCallBack<T> callBack) {
		get(url, null, callBack);
	}

	public <T> void get(String url, AjaxParams params,
			FzCallBack<T> callBack) {
		sendRequest(httpClient, httpContext,
				new HttpGet(getUrlWithQueryString(url, params)), null, callBack);
	}

	public <T> void get(String url, Header[] headers, AjaxParams params,
			FzCallBack<T> callBack) {
	...
	// ------------------post 请求-----------------------
	public <T> void post(String url, FzCallBack<T> callBack) {
		post(url, null, callBack);
	}

	public <T> void post(String url, AjaxParams params,
			FzCallBack<T> callBack) {
		post(url, paramsToEntity(params), null, callBack);
	}

	public <T> void post(String url, HttpEntity entity, String contentType,
			FzCallBack<T> callBack) {
		sendRequest(httpClient, httpContext,
	...
	public <T> void post(String url, Header[] headers, AjaxParams params,
			String contentType, FzCallBack<T> callBack) {
	...
	public <T> void post(String url, Header[] headers, HttpEntity entity,
			String contentType, FzCallBack<T> callBack) {
	...
	public void put(String url, FzCallBack<? extends Object> callBack) {
		put(url, null, callBack);
	}

	public void put(String url, AjaxParams params,
			FzCallBack<? extends Object> callBack) {
	...
	public void put(String url, Header[] headers, HttpEntity entity,
			String contentType, FzCallBack<? extends Object> callBack) {
		HttpEntityEnclosingRequestBase request = addEntityToRequestBase(
	...
	// ------------------delete 请求-----------------------
	public void delete(String url, FzCallBack<? extends Object> callBack) {
	...
	// ---------------------下载---------------------------------------
	public FzHttpHandler<File> download(String url, String target,
			FzCallBack<File> callback) {
		return download(url, null, target, false, callback);
	}

	public FzHttpHandler<File> download(String url, String target,
			boolean isResume, FzCallBack<File> callback) {
		return download(url, null, target, isResume, callback);
	}

	public FzHttpHandler<File> download(String url, AjaxParams params,
			String target, FzCallBack<File> callback) {
		return download(url, params, target, false, callback);
	}

	public FzHttpHandler<File> download(String url, AjaxParams params,
			String target, boolean isResume, FzCallBack<File> callback) {
		final HttpGet get = new HttpGet(getUrlWithQueryString(url, params));
		FzHttpHandler<File> handler = new FzHttpHandler<File>(httpClient,
	...
	protected <T> void sendRequest(DefaultHttpClient client,
			...
			String contentType, FzCallBack<T> FzCallBack) {
		if (contentType != null) {
			uriRequest.addHeader("Content-Type", contentType);
			uriRequest.addHeader("Accept", "application/json");
		}
//		if (mHandler==null)
//		{
//			mHandler  = new FzHttpHandler<T>(client, httpContext, FzCallBack, charset);
//		}
//		mHandler.executeOnExecutor(executor, uriRequest);
		new FzHttpHandler<T>(client, httpContext, FzCallBack, charset)
...
//	}
	...
	{
		super.finalize();
		System.out.println(FzHttpReq.class.getSimpleName()+"-->被回收了");
	}
