...
import com.fz.afinal.bitmap.core.BitmapCache;
import com.fz.afinal.bitmap.core.BitmapDisplayConfig;
import com.fz.afinal.bitmap.core.BitmapProcess;
import com.fz.afinal.bitmap.display.CicleBitmapDisplayer;
import com.fz.afinal.bitmap.display.Displayer;
import com.fz.afinal.bitmap.display.RoundedBitmapDisplayer;
import com.fz.afinal.bitmap.display.CicleBitmapDisplayer.CicleDrawable;
import com.fz.afinal.bitmap.display.RoundedBitmapDisplayer.RoundedDrawable;
import com.fz.afinal.bitmap.display.SimpleDisplayer;
import com.fz.afinal.bitmap.download.Downloader;
import com.fz.afinal.bitmap.download.SimpleDownloader;
import com.fz.afinal.core.AsyncTask;
import com.fz.afinal.utils.FinalUtils;
...
public class FinalBitmap
{
	...
	private FinalBitmap(Context context)
	{
		...
		configDiskCachePath(FinalUtils.getDiskCacheDir(context, "fzCache").getAbsolutePath());// 配置缓存路径
	...
	{
		doDisplay(imageView, uri, displayer, null);
	}
	...
	public void display(View imageView, String uri, boolean isCicle)
	{
		BitmapDisplayConfig displayConfig = null;
		if (displayConfig == null)
		{
			displayConfig = getDisplayConfig();
			displayConfig.setCirle(true);
		}
		doDisplay(imageView, uri, displayConfig);
	}
	...
	private void doDisplay(View imageView, String uri, Displayer displayer, BitmapDisplayConfig displayConfig)
	{
		if (!mInit)
		{
			init();
		...
		if (bitmap != null)
		{
			if (displayer != null)
			{
				if (displayer instanceof SimpleDisplayer)
				{
				...
				else if (displayer instanceof RoundedBitmapDisplayer)
				{
					RoundedDrawable drawable = new RoundedBitmapDisplayer.RoundedDrawable(bitmap,
							((RoundedBitmapDisplayer) displayer).getRoundPixels(), 0);
					if (imageView instanceof ImageView)
					{
				...
				}else if (displayer instanceof CicleBitmapDisplayer){
		...
		{
			final BitmapLoadAndDisplayTask task = new BitmapLoadAndDisplayTask(imageView, displayConfig, displayer);
			// 设置默认图片
	...
	{
		if (mBitmapProcess != null) { return mBitmapProcess.getBitmap(uri, config); }
		return null;
	}
	...
	private class BitmapLoadAndDisplayTask extends AsyncTask<Object, Void, Bitmap>
	{
		...
		public BitmapLoadAndDisplayTask(View imageView, BitmapDisplayConfig config, Displayer diaplayer)
		{
			imageViewReference = new WeakReference<View>(imageView);
			displayConfig = config;
			this.mDisplayer = diaplayer;
		}
		...
		protected void onPostExecute(Bitmap bitmap)
		{
			...
			if (bitmap != null && imageView != null  )
			{
				if (mDisplayer != null)
				{
					mDisplayer.loadCompletedisplay(imageView, bitmap, displayConfig);
				}
				else
				{
			...
			else if (bitmap == null && imageView != null)
			{
				if (mDisplayer != null)
				{
					mDisplayer.loadFailDisplay(imageView, displayConfig.getLoadfailBitmap());
				}
				else
				{
